...
func readEnvVars() (string, string, string, string, string) {
	qTestServer := os.Getenv("QTEST_SERVER")
	if qTestServer == "" {
		log.Fatalln("Environment variable QTEST_SERVER not specified")
	}
	...
	return qTestServer, username, password, projectName, suiteName
}
...
func processResultsToQTest(j Testsuites, client *qtest.Client, projectID int, suiteID int) {
	logJunitDetail(j)
	...
	for _, ts := range j.Testsuite {
		for i, tc := range ts.Testcase {
			...
			var tcStatus int
			if tc.Failure.Text != "" {
				tcStatus = qtest.StatusFailed
			} else if tc.Skipped != nil {
				//tcStatus = qtest.StatusUntested  //StatusUntested results in a failure when posting - library bug??
				tcStatus = qtest.StatusRetest
			} else {
				tcStatus = qtest.StatusPassed
			}

			tsr := qtest.SendableResult{
				//Elapsed: *qtest.TimespanFromDuration(duration),
				StatusID: tcStatus,
...
func getProjectID(client *qtest.Client, projectName string) (int, error) {
...
func getSuiteID(client *qtest.Client, projectID int, suiteName string) (int, error) {
...
func getTestCaseID(client *qtest.Client, projectID int, suiteID int, testcaseName string) (int, error) {
...
func addTestCase(client *qtest.Client, suiteID int, testcaseName string) (qtest.Case, error) {
	now := time.Now().Format("2006-01-02 15:04:05")
	newTestCase := qtest.SendableCase{
		Title: testcaseName,
		Date:  now,
	}
...
func main() {
	qTestServer, username, password, projectName, suiteName := readEnvVars()
	junitDoc := readJunitXML(os.Stdin)
	client := qtest.NewClient(qTestServer, username, password)
	...
	processResultsToQTest(junitDoc, client, projectID, suiteID)
